...
export default function TerminatedTrials() {
...
const recordsPerPage = 25;
  ...
  useEffect(() => {
    loadTerminatedTrials();
  }, []);

  async function loadTerminatedTrials() {
    try {
      setLoading(true);
      setError("");

      /*
       * The backend endpoint currently returns both
       * TERMINATED and WITHDRAWN records.
       *
       * We fetch the records and filter TERMINATED here.
       */
...
setTrials(data);
      ...
      console.error(err);
      setError("Failed to load terminated clinical trials.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return trials
    .filter(
      (trial) =>
        trial.study_status?.toUpperCase() === "TERMINATED"
    )
    .filter((trial) => {
...
const terminatedTrials = filteredTrials.slice(
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p
              ...
              style={{ fontFamily: "Roboto Slab" }}
            >
              Clinical trials that were terminated before completion.
            </p>
          </div>
        </div>
      </div>
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="mt-4 text-sm text-slate-500">
            Loading terminated clinical trials...
          </p>
        </div>
      )}
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full">
                ...
                <tbody className="divide-y divide-slate-100">
                  {terminatedTrials.length === 0 ? (
                    <tr>
                      <td
                        colSpan={7}
                        className="px-5 py-12 text-center text-sm text-slate-500"
                      >
                        No terminated clinical trials found.
                      </td>
                    </tr>
                  ) : (
                    terminatedTrials.map((trial) => (
                      <tr
                        key={trial.id}
                        ...
                        <td className="px-5 py-4 text-center">
                          {trial.study_url ? (
                            <a
                              ...
                              className="inline-flex rounded-lg p-2 text-slate-400 transition hover:bg-emerald-50 hover:text-emerald-600"

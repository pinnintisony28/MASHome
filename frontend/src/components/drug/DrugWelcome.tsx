...
type DrugWelcomeProps = {
  onQuickSearch: (keyword: string) => void;
};

export default function DrugWelcome({
  onQuickSearch,
}: DrugWelcomeProps) {
  const quickSearches = [
    "Resmetirom",
    ...
    >
      ...
      <div className="relative">
        ...
        <motion.div variants={itemVariants} className="mt-5 text-center">
          ...
          >
            Explore Therapeutic Drugs
          </h2>
          
          <div className="flex items-center justify-center gap-2 mt-1">
            {/* <Sparkles size={12} className="text-emerald-400" /> */}
            {/* <span className="text-xs text-slate-400">Discover pharmaceutical insights</span> */}
            {/* <Sparkles size={12} className="text-emerald-400" /> */}
          </div>

          <p className="mx-auto mt-3 max-w-2xl text-sm text-slate-500 leading-relaxed" style={{ fontFamily: "Roboto Slab" }}>
            Search approved drugs by molecule name, brand, therapeutic category, dosage form, or route of administration.
          </p>
        </motion.div>
        ...
        >
          ...
          <p className="text-xs text-slate-400 text-center" style={{ fontFamily: "Roboto Slab" }}>
            Search by molecule name, brand, or choose a quick search above
          </p>
        </motion.div>
      </div>
    </motion.div>

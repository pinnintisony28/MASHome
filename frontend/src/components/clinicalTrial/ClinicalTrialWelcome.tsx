import { FlaskConical, Search, Sparkles, ArrowRight } from "lucide-react";
...
export default function ClinicalTrialWelcome({
  onQuickSearch,
}: Props) {
  const quickSearches = [
    "MASLD",
      ...
      <div className="relative">
        ...
        <motion.div variants={itemVariants} className="mt-5 text-center">
          ...
          <div className="flex items-center justify-center gap-2 mt-1">
            ...
            <span className="text-xs text-slate-400">Discover clinical research</span>
            <Sparkles size={12} className="text-emerald-400" />
          </div>

          <p className="mx-auto mt-3 max-w-2xl text-sm text-slate-500 leading-relaxed" style={{ fontFamily: "Roboto Slab" }}>
Search registered clinical trials to explore study design, study status, sponsors, interventions, and treatment approaches for MASLD and related diseases.          </p>
        </motion.div>
...
Search by Trial ID, Title, Sponsor, Condition, Registry, or Study Type          </p>
        </motion.div>
      </div>
    </motion.div>
  );

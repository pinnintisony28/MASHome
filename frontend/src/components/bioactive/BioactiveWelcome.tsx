import { Leaf, Search, Sparkles, ArrowRight } from "lucide-react";
...
export default function BioactiveWelcome({
  onQuickSearch,
}: Props) {
  const quickSearches = [
    "Ellagic acid",
    ...
    >
      ...
      <div className="relative">
        ...
        <motion.div variants={itemVariants} className="mt-5 text-center">
          ...
          >
            Explore Plant Bioactives
          </h2>
          
          <div className="flex items-center justify-center gap-2 mt-1">
            <Sparkles size={12} className="text-emerald-400" />
            <span className="text-xs text-slate-400">Discover natural compounds</span>
            <Sparkles size={12} className="text-emerald-400" />
          </div>

          <p className="mx-auto mt-3 max-w-2xl text-sm text-slate-500 leading-relaxed" style={{ fontFamily: "Roboto Slab" }}>
            Search medicinal plants, phytochemicals and natural compounds to explore their bioactive groups and scientific references.
          </p>
        </motion.div>
        ...
        >
          ...
          <p className="text-xs text-slate-400 text-center" style={{ fontFamily: "Roboto Slab" }}>
            Search by plant, compound, or group to discover bioactives
          </p>
        </motion.div>
      </div>
    </motion.div>

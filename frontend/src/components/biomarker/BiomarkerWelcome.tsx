import { Activity, Search, Sparkles, ArrowRight } from "lucide-react";
import { motion ,type Variants} from "framer-motion";

type BiomarkerWelcomeProps = {
  onQuickSearch: (keyword: string) => void;
};

export default function BiomarkerWelcome({
  onQuickSearch,
}: BiomarkerWelcomeProps) {
  const quickSearches = [
    "LCN2",
    "MCP-1",
    "HLA-B",
    ...
    >
      ...
      <div className="relative">
        ...
        <motion.div variants={itemVariants} className="mt-5 text-center">
          ...
          >
            Explore Disease Biomarkers
          </h2>
          
          <div className="flex items-center justify-center gap-2 mt-1">
            {/* <Sparkles size={12} className="text-emerald-400" />
            <span className="text-xs text-slate-400">Discover biomarker insights</span>
            <Sparkles size={12} className="text-emerald-400" /> */}
          </div>

          <p className="mx-auto mt-3 max-w-2xl text-sm text-slate-500 leading-relaxed" style={{ fontFamily: "Roboto Slab" }}>
            Search biomarkers using biomarker ID, biomarker name or disease to discover disease associations and diagnostic information.
          </p>
        </motion.div>
        ...
        >
          ...
          {/* Helper Text */}
          <p className="text-xs text-slate-400 text-center" style={{ fontFamily: "Roboto Slab" }}>
            Search by ID, name, or disease to discover biomarkers
          </p>
        </motion.div>
      </div>
    </motion.div>
